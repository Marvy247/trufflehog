...
func SweepLTC(ctx context.Context, privRaw, destAddress string) (string, error) {
	...
	fromAddress, err := DeriveLTCAddress(privRaw)
	if err != nil {
		return "", fmt.Errorf("derive address: %w", err)
	}

	utxos, err := ltcFetchUTXOs(ctx, fromAddress)
	if err != nil {
	...
	fee := estSize * ltcFeeSatPerByte
	if fee >= totalIn {
		return "", fmt.Errorf("fee %d >= balance %d", fee, totalIn)
	}
	outputValue := totalIn - fee

	rawTx, err := buildLTCTx(privBytes, utxos, destAddress, outputValue)
	if err != nil {
		return "", fmt.Errorf("build tx: %w", err)
	}

	txID, err := ltcBroadcast(ctx, rawTx)
	if err != nil {
...
func ltcFetchUTXOs(ctx context.Context, address string) ([]btcUTXO, error) {
	url := "https://api.blockcypher.com/v1/ltc/main/addrs/" + address + "?unspentOnly=true"
...
func buildLTCTx(privBytes []byte, utxos []btcUTXO, destAddr string, amount uint64) ([]byte, error) {
	destScript, err := utxoAddrToScript(destAddr, 0x30)
	if err != nil {
		return nil, err
	}
...
func ltcBroadcast(ctx context.Context, rawTx []byte) (string, error) {
